...
from extract_buy_owner_details import extract_basic_details, extract_phone_number
    ...
    with sync_playwright() as p:
        ...
        if len(state["rows"]) > 0:
            if 'Property URL' in state["rows"][0]:
                ...
                url_col = 'Number'
            elif 'Owner Name' in state["rows"][0] and str(state["rows"][0]['Owner Name']).startswith('http'):
                 url_col = 'Owner Name'
        ...
        while True:
            ...
            if action == "navigate":
                ...
                if 0 <= idx < len(state["rows"]):
                    ...
                    url = row_data.get("Property URL", row_data.get("Number", row_data.get("Owner Name", "")))
                ...
                idx = cmd.get("index")
                if 0 <= idx < len(state["rows"]):
                    state["status"] = f"Extracting basic details for row {idx+1}..."
                    try:
                ...
                if 0 <= idx < len(state["rows"]):
                    ...
                    try:
                        ...
                        if data.get("Number") != "Extraction Failed":
                            ...
                            # Update url col to actual number
                            df = pd.DataFrame(state["rows"])
                            actual_url_col = 'Property URL' if 'Property URL' in df.columns else 'Number'
                            
                            state["rows"][idx][actual_url_col] = extracted_num
...
@app.route('/')
def index():
    return render_template('buy_owner.html')
...
def load_file():
    ...
    try:
        ...
        url_col = 'Property URL' if 'Property URL' in df.columns else 'Number' if 'Number' in df.columns else 'Owner Name'
        if url_col not in df.columns:
            return jsonify({"success": False, "error": f"Missing URL column in {file_path}. Required either 'Property URL' or 'Number' or 'Owner Name'."})
            
        new_cols = [
            "Phone Number", "Owner Name", "Configuration", "Price", 
            "Area", "Address", "Floor Number", "Facing", "Overlooking", 
...
def get_status():
    ...
    if 0 <= state["current_index"] < state["total"]:
        ...
        safe_state["current_row_data"] = {
            "Phone Number": row_data.get("Phone Number", "N/A"),
            ...
            "Possession": row_data.get("Possession in", "N/A")
        }
    else:
...
if __name__ == '__main__':
    app.run(debug=True, port=5002, use_reloader=False)  # Using 5002 to avoid collision with Rent GUI
