...
def extract_properties(location="Thane"):
    print(f"Starting connection to extract properties for {location}...")
    ...
    with sync_playwright() as p:
        try:
            ...
            # Select 'Rent' mode instead of 'Buy' if present
            print("Switching to 'Rent' mode...")
            try:
                page.get_by_text("Rent", exact=True).first.click(timeout=5000)
                time.sleep(1)
            except Exception as e:
                print(f"Notice: Could not click Rent tab. Exploring directly. Error: {e}")
            ...
            if properties_data:
                ...
                try:
                    excel_filename = f"All_Properties_{location}_Formatted.xlsx"
                    df.to_excel(excel_filename, index=False)
                    print(f"\nSUCCESS! Structured Excel file saved: {excel_filename}")
                except Exception as e:
                    print(f"\nNote: Could not save Excel file. Reason: {e}")
                
                csv_filename = f"{location}_Extracted_Properties.csv"
...
if __name__ == "__main__":
    ...
    out_file = extract_properties(loc)
